...
WORK="$(mktemp -d /tmp/tde2e-publish.XXXXXX)"
cleanup() {
...
if [ "${TEST_BUILD}" = "1" ]; then
    ...
    ( cd "${BINARY_BUILD_DIR}" && debuild -b -d -us -uc )
    say "Binary test build succeeded"

    say "Installed layout"
    find "${BINARY_BUILD_DIR}/debian/libtde2e-dev" -type f \
        ! -name '*.h' | sed "s|${BINARY_BUILD_DIR}/debian/libtde2e-dev||"
    printf 'headers: %s\n' \
        "$(find "${BINARY_BUILD_DIR}/debian/libtde2e-dev" -name '*.h' 2>/dev/null | wc -l)"
fi
