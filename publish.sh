#!/usr/bin/env bash
# Host-side launcher for building and publishing tde2e to Launchpad.
...
# td tree *is* the source. To move to a newer upstream commit, rebase this
...
IMAGE_TAG="${IMAGE_TAG:-tde2e-ppa-builder}"
...
Build a source-only tde2e package for ${SERIES} in an Ubuntu Podman
...
[ "${BRANCH}" = "tde2e_ubuntu" ] \
    || warn "on branch ${BRANCH}, expected tde2e_ubuntu"
...
[ "${SOURCE}" = "tde2e" ] || die "unexpected source package: ${SOURCE}"
[ "${DISTRIBUTION}" = "${SERIES}" ] \
...
if podman image exists "${IMAGE_TAG}"; then
    IMAGE_CONTAINERFILE_SHA256="$(
        podman image inspect \
            --format '{{ index .Labels "org.tde2e.containerfile-sha256" }}' \
...
if [ "${IMAGE_CONTAINERFILE_SHA256}" != "${CONTAINERFILE_SHA256}" ]; then
    ...
    podman build \
        --label "org.tde2e.containerfile-sha256=${CONTAINERFILE_SHA256}" \
        --tag "${IMAGE_TAG}" \
        --file "${CONTAINERFILE}" \
        "${PACKAGE_DIR}"
fi
...
if [ "${UNSIGNED}" -eq 0 ]; then
    ...
    TEMP_GPG="$(mktemp -d "${TMPDIR:-/tmp}/tde2e-gnupg.XXXXXX")"
